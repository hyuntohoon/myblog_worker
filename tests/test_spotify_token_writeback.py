...
#
# Pure-mock — httpx and boto3 SSM are faked. The write-back is an SSM Parameter
# Store side-effect that touches no DB transaction boundary, so (unlike the
...
class _FakeSSM:
    """A fake SSM client; records every put and serves a mutable SecureString payload."""
    ...
    def get_parameter(self, Name, WithDecryption=False):
        assert WithDecryption is True, "the spotify secret is a SecureString"
        return {"Parameter": {"Value": json.dumps(self.stored)}}

    def put_parameter(self, Name, Value, Type, Overwrite):
        self.stored = json.loads(Value)
        self.puts.append(self.stored)
        self.put_kwargs.append({"Name": Name, "Type": Type, "Overwrite": Overwrite})


@pytest.fixture
def ssm(monkeypatch):
    """Point settings + boto3 at a fake SSM Parameter Store holding /myblog/spotify."""
    fake = _FakeSSM(dict(_CREDS))
    monkeypatch.setattr(settings, "SPOTIFY_SECRETS_PARAM", "/myblog/spotify")
...
@pytest.mark.unit
def test_refresh_success_stamps_timestamp_and_clears_reauth(ssm, monkeypatch):
    ssm.stored["needs_reauth"] = True  # a prior invalid_grant had flagged it
...
@pytest.mark.unit
def test_refresh_rotated_token_persisted_and_cached_in_memory(ssm, monkeypatch):
    monkeypatch.setattr(
        suc.httpx, "request",
    ...
    assert ssm.puts[-1]["refresh_token"] == "NEW"    # and persisted to the secret
    assert "last_successful_refresh_at" in ssm.puts[-1]
    # CHORE-secrets-ssm-migration: the write must stay a SecureString overwrite of
    # the same parameter — a plain String would store the refresh token unencrypted.
    assert ssm.put_kwargs[-1] == {
        "Name": "/myblog/spotify",
        "Type": "SecureString",
        "Overwrite": True,
    }
...
@pytest.mark.unit
def test_invalid_grant_flags_needs_reauth_and_raises(ssm, monkeypatch):
...
@pytest.mark.unit
def test_transient_5xx_does_not_flip_reauth(ssm, monkeypatch):
    ...
    assert ssm.puts == []  # a transient failure must never trip "재인증 필요"


# ── _persist_token_state edge cases ───────────────────────────────────────────────

@pytest.mark.unit
def test_persist_is_noop_without_param(monkeypatch):
    monkeypatch.setattr(settings, "SPOTIFY_SECRETS_PARAM", "")
...
@pytest.mark.unit
def test_persist_needs_reauth_is_idempotent(ssm):
    ssm.stored["needs_reauth"] = True
    _persist_token_state(needs_reauth=True)
    assert ssm.puts == []  # already flagged → don't churn a new secret version each tick


@pytest.mark.unit
def test_write_back_failure_is_non_fatal(ssm, monkeypatch):
    def _boom(**kwargs):
        raise RuntimeError("ssm throttled")

    ssm.put_parameter = _boom
    monkeypatch.setattr(suc.httpx, "request", _post({"access_token": "AT", "expires_in": 3600}))
